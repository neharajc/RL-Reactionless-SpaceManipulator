...
class Hybrid5DOFEnv(gym.Env):
    ...
    def compute_reward(self, agent_action):
        ...
        # -------------------------------
        # Joint error (target = 0 rad)
        # -------------------------------
        joint_error = np.sum(q[1:] ** 2)

        # -------------------------------
        # Penalize leaving ±10 degrees
        # -------------------------------
        angle_limit = np.deg2rad(10.0)
        limit_penalty = np.sum(
            np.maximum(0, np.abs(q[1:]) - angle_limit) ** 2
        )

        # -------------------------------
        # Base stability penalty
        # -------------------------------
...
class ConvergenceCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        if self.num_timesteps % self.eval_freq == 0:
            print(f"\n[Eval {self.num_timesteps}] Rendering rollout...")
