...
def main():
    ...
    # --- Plot Joint 1: Desired vs Commanded vs Actual Velocity ---
    plt.figure(figsize=(10, 6))
    plt.plot(time_vec, dq1_traj, 'k--', label="Joint1 Desired Vel")
    plt.plot(time_vec, [vc[0] for vc in vcmd_log], 'r-', label="Joint1 Commanded Vel")
    plt.plot(time_vec, dq_log[:, 1], 'b', label="Joint1 Actual Vel")
    plt.xlabel("Time [s]")
    plt.ylabel("Velocity [rad/s]")
    plt.title("Joint 1: Desired vs Commanded vs Actual Velocity")
    plt.legend()
    plt.grid(True)
    plt.show()
