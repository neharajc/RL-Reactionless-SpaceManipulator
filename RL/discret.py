...
with mujoco.viewer.launch_passive(model, data) as viewer:
    for ep in range(episodes):
        ...
        # Randomize starting angles for joint1 & joint2
        data.qpos[j1_id] = np.radians(np.random.uniform(-180, 180))
        data.qpos[j2_id] = np.radians(np.random.uniform(-180, 180))
        ...
        for step in range(steps):
            ...
            # Compensating torques on joint3 & joint4
            data.qfrc_applied[j3_id] = -tau1
            data.qfrc_applied[j4_id] = -tau2
            ...
            err1 = abs((j1 % 360) - 0)
            err2 = abs((j2 % 360) - 0)
            reward = math.exp(-(err1 + err2) / 20.0)  # ~1 near 0, ~0 when far
