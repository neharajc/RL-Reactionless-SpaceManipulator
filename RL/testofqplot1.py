...
# -------------------------
# Base joint DOF (CORRECT)
# -------------------------
base_joint_id = model.joint("base_joint").id
base_dof = model.joint("base_joint").dofadr[0]

print("Base joint name :", model.joint(base_joint_id).name)
print("Base DOF index  :", base_dof)
print("Base joint type :", model.jnt_type[base_joint_id],
...
def test_agent(steps=800, target1=0.0, target2=0.0):
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for step in range(steps):
            ...
            # -------------------------
            # CORRECT base velocity logging
            # -------------------------
            base_vel_log.append(data.qvel[base_dof])
    ...
    plt.xlabel("Step")
    plt.ylabel("Base Angular Velocity (rad/s)")
