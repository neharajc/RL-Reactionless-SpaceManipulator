...
class Hybrid5DOFEnv(gym.Env):
    ...
    def __init__(self, xml_path=XML_PATH, render=False, seed: int = None):
        ...
        self.joint_names = [mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, i) or f"joint_{i}" for i in range(self.nq)]
        self.actuator_names = [mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, i) or f"actuator_{i}" for i in range(self.nu)]
    ...
    def compute_reward(self, agent_action):
        ...
        reward = - self.w_joint * joint_error - self.w_base * base_pen - self.w_action * action_pen - self.w_step
    ...
    def step(self, action):
        ...
        for i_agent, act_idx in enumerate(self.exposed_act_idxs):
            ctrl[act_idx] = action[i_agent] * self.ctrl_max[act_idx]

        if self.base_act_idx is not None:
            ctrl[self.base_act_idx] = 0.0
        ...
        # ✅ PRINT QPOS EVERY 10 STEPS
        if self.step_count % 10 == 0:
            print("\nStep:", self.step_count)
            for i in range(self.nq):
                print(f"{self.joint_names[i]:15s}: {self.data.qpos[i]:.5f} rad")
...
class ConvergenceCallback(BaseCallback):
    def __init__(self, eval_env, eval_freq=50000, warmup_steps=40000):
        super().__init__()
    ...
    def _on_step(self):
        ...
        if self.num_timesteps % self.eval_freq == 0:
            ...
            print(f"[Eval {self.num_timesteps}] Mean reward = {mean_reward:.3f}")
...
#                           TRAINING                                      #
# ======================================================================= #
def make_env_fn(xml_path=XML_PATH, seed=SEED, render=False):
    def _init():
        return Monitor(Hybrid5DOFEnv(xml_path=xml_path, render=render, seed=seed))
    return _init
...
def train():
    ...
    env = DummyVecEnv([make_env_fn(XML_PATH, SEED, render=False)])
    eval_env = DummyVecEnv([make_env_fn(XML_PATH, SEED+1, render=True)])
