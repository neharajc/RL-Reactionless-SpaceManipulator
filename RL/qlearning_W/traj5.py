...
data.qpos[0]=np.radians(q_start[0])
data.qpos[1]=np.radians(q_start[1])
data.qpos[2]=np.radians(q_start[2])
data.qpos[3]=np.radians(q_start[3])
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    for step in range(steps):
        ...
        error_joint1.append(abs(current_joint1 -  pos[0]))
        error_joint2.append(abs(current_joint2 -  pos[1]))
