...
class QLearnerSparse:
    ...
    def update(self, s_key, a_idx, reward, s_next_key, done):
        ...
        if done:
            ...
            target = reward + self.gamma * max(self.get_q(s_next_key, idx)
...
# Convergence Checker
# -------------------------
class QConvergenceChecker:
    def __init__(self, threshold=1e-4, patience=150):
        self.threshold = threshold
        self.patience = patience
        self.counter = 0
        self.prev_vals = None

    def check(self, q_dict):
        items = sorted(q_dict.items())
        current_vals = np.array([v for (_,v) in items])

        if self.prev_vals is None:
            self.prev_vals = current_vals
            return False

        # if size changed (new states discovered)
        if len(current_vals) != len(self.prev_vals):
            self.prev_vals = current_vals
            self.counter = 0
            return False

        diff = np.sum(np.abs(current_vals - self.prev_vals))
        self.prev_vals = current_vals
...
# Training
# -------------------------
def train_demo(episodes=50000, steps=750):

    agent = QLearnerSparse(ACTIONS)
    convergence = QConvergenceChecker(threshold=1e-4, patience=150)
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for ep in range(episodes):
            ...
            # -----------------------------
            if convergence.check(dict(agent.Q)):
                print("\n🟢 Q-table converged! Stopping training early.")
                break

    with open("q_table_8123.pkl", "wb") as f:
